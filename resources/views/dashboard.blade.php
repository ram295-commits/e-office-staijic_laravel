    ...
    <div x-show="selectedMail" 
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
                        ...
                        <template x-for="disp in (selectedMail ? selectedMail.dispositions : [])" :key="disp.id">
                            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3.5 space-y-2">
                                ...
                                <div x-show="disp.response_notes" class="mt-2 bg-emerald-50 border border-emerald-100 p-2.5 rounded text-xs text-emerald-950">
                                    ...
                                    <!-- Attachment display -->
                                    <div x-show="disp.response_attachment_path" class="mt-2 flex items-center gap-1.5 bg-white border border-emerald-100 p-1.5 rounded">
                                        <i class="ph ph-paperclip text-emerald-700"></i>
                                        <a :href="disp.response_attachment_path" target="_blank" class="font-bold text-[10px] text-primary hover:underline truncate" x-text="disp.response_attachment_name || 'Download Lampiran Respon'"></a>
                                    </div>
            ...
            <div x-show="dispositionFormOpen" 
                ...
                <form action="{{ route('dispositions.store') }}" method="POST" class="space-y-4 text-xs" x-data="{
                    dropdownOpen: false,
                    search: '',
                    users: {{ json_encode($users->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'role' => $u->position ?? 'Staff'])) }},
                    selected: [],
                    toggleUser(id) {
                    ...
                    getSelectedNames() {
                        if (this.selected.length === 0) return '— Pilih Penerima —';
                        return this.users.filter(u => this.selected.includes(u.id)).map(u => u.name).join(', ');
                    }
                }" @submit="if (selected.length === 0) { event.preventDefault(); alert('Pilih minimal satu penerima disposisi.'); }" @click.away="dropdownOpen = false">
                    @csrf
                    ...
                    <div style="position: relative;">
                        ...
                        <template x-for="userId in selected" :key="userId">
                            <input type="hidden" name="to_user_ids[]" :value="userId">
                        </template>
                        
                        <!-- Toggle Button -->
                        <button type="button" @click="dropdownOpen = !dropdownOpen" class="w-full text-left flex justify-between items-center border border-gray-300 rounded-lg px-3 py-2 bg-white text-gray-700 font-semibold" style="text-align: left; display: flex; justify-content: space-between; align-items: center;">
                            <span class="truncate" x-text="getSelectedNames()" style="max-width: 90%;"></span>
                            <i class="ph ph-caret-down text-gray-400"></i>
                        </button>
                        
                        <!-- Dropdown Box -->
                        <div x-show="dropdownOpen" class="absolute left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-2" style="display: none; position: absolute; max-height: 200px; overflow-y: auto;">
                            <!-- Search -->
                            <div class="relative mb-2">
                                <span class="absolute inset-y-0 left-0 pl-2 flex items-center pointer-events-none">
                                    <i class="ph ph-magnifying-glass text-gray-400 text-sm"></i>
                                </span>
                                <input type="text" x-model="search" placeholder="Cari pegawai..." class="w-full pl-7 pr-3 py-1 border border-gray-200 rounded-lg text-xs bg-gray-50 focus:outline-none" style="padding-left: 24px; height: 28px;">
                            </div>
                            
                            <!-- List -->
                            <div class="space-y-0.5">
                                <template x-for="u in filteredUsers" :key="u.id">
                                    <label class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-gray-50 cursor-pointer text-xs" style="display: flex; align-items: center; gap: 6px; padding: 4px 6px; cursor: pointer;">
                                        <input type="checkbox" :value="u.id" :checked="selected.includes(u.id)" @change="toggleUser(u.id)" class="rounded text-primary focus:ring-primary border-gray-300" style="cursor: pointer; width: 13px; height: 13px;">
                                        <div style="display: flex; flex-direction: column;">
                                            <span class="font-bold text-gray-800" x-text="u.name"></span>
                                            <span class="text-[9px] text-gray-400" x-text="u.role"></span>
                                        </div>
                                    </label>
                                </template>
                                <div x-show="filteredUsers.length === 0" class="text-center py-3 text-xs text-gray-400">
