...
<div style="display:grid;grid-template-columns:1fr 340px;gap:20px;align-items:start">
    ...
    <div>
        ...
        <div class="card">
            ...
            <div style="padding:14px;border:1px solid var(--border);border-radius:8px;margin-bottom:10px">
                ...
                @if($disp->response_notes)
                    <div style="margin-top:8px;padding:10px;background:rgba(34,197,94,0.05);border:1px solid rgba(34,197,94,0.2);border-radius:6px;font-size:13px;color:var(--success)">
                        ...
                        @if($disp->response_attachment_path)
                            <div style="margin-top:6px; display:flex; align-items:center; gap:6px;">
                                <i class="ph ph-paperclip"></i>
                                <a href="{{ asset('storage/' . $disp->response_attachment_path) }}" target="_blank" style="text-decoration:underline; color:#1e5c45; font-weight:600;">
                                    {{ $disp->response_attachment_name ?? 'Download Lampiran Respon' }}
                                </a>
                            </div>
                        @endif
                    </div>
            ...
            <form method="POST" action="{{ route('dispositions.store') }}" x-data="{
                open: false,
                search: '',
                users: {{ json_encode($users->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'role' => $u->role_label])) }},
                selected: [],
                toggleUser(id) {
            ...
            }" @submit="if (selected.length === 0) { event.preventDefault(); alert('Pilih minimal satu penerima disposisi.'); }" @click.away="open = false">
                @csrf
                <input type="hidden" name="mail_id" value="{{ $mail->id }}">
                <div class="form-grid-2">
                    <div class="form-group" style="position: relative;">
                        <label class="form-label">Ditujukan kepada <span class="req">*</span></label>
                        ...
                        <button type="button" @click="open = !open" class="form-control text-left flex justify-between items-center bg-white" style="text-align: left; display: flex; justify-content: space-between; align-items: center; width: 100%;">
                            <span class="truncate" x-text="getSelectedNames()" style="max-width: 90%;"></span>
                            <i class="ph ph-caret-down text-gray-400"></i>
                        </button>

                        <!-- Dropdown Panel -->
                        <div x-show="open" class="absolute left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-2" style="display: none; position: absolute; max-height: 250px; overflow-y: auto;">
                            <!-- Search -->
                            <div class="relative mb-2">
                                ...
                                <input type="text" x-model="search" placeholder="Cari pegawai..." class="form-control pl-8 py-1.5 text-xs bg-gray-50 border-gray-200" style="padding-left: 28px; height: 32px; font-size: 11px;">
                            </div>

                            <!-- List -->
                            <div class="space-y-0.5">
                                <template x-for="u in filteredUsers" :key="u.id">
                                    <label class="flex items-center gap-2.5 px-2.5 py-2 rounded-md hover:bg-gray-50 cursor-pointer text-xs" style="display: flex; align-items: center; gap: 8px; padding: 6px 10px; cursor: pointer;">
                                        <input type="checkbox" :value="u.id" :checked="selected.includes(u.id)" @change="toggleUser(u.id)" class="rounded text-primary focus:ring-primary border-gray-300" style="cursor: pointer; width: 14px; height: 14px;">
                                        <div style="display: flex; flex-direction: column;">
                                            <span class="font-semibold text-gray-800" x-text="u.name"></span>
                                            <span class="text-[10px] text-gray-400" x-text="u.role"></span>
                                        </div>
                                    </label>
                                </template>
                                <div x-show="filteredUsers.length === 0" class="text-center py-4 text-xs text-gray-400">
